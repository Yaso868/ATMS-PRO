...
function renderLiveDispositionTargetBlock(driver,settings,consent){
  const shell=ensureLiveDispositionTargetBlock();if(!shell)return;
  ...
  const meta=$('atmsLiveTargetDriverMeta');if(meta)meta.textContent=driver?(driver.vehicle?`Fahrzeug: ${driver.vehicle}`:'Fahrzeug nicht hinterlegt'):'Kein Fahrer verfügbar';
  ...
  const session=getDriverSession(),geo=settings?.lastGeo&&driver&&settings.lastGeo.driverId===driver.id?settings.lastGeo:null;const active=Boolean(driver&&session.active&&session.driverId===driver.id&&consent);let state='Zustimmung offen';
  if(consent&&session.active&&session.driverId===driver?.id)state=geo?'● Tracking aktiv':'● Tracking gestartet · Position ausstehend';else if(consent)state='● Tracking freigegeben · Schicht nicht gestartet';
