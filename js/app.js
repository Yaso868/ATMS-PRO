...
function renderLiveDispositionTargetPositionInfo(driver,settings,consent){
  ...
  if(body){body.className='atms-live-target-position-body '+(geo?'is-live':'');body.innerHTML=geo?`<div class="atms-live-target-geo-dot"></div><b>GPS-Position verfügbar</b><span>${esc(atmsLiveTargetRelativeTime(geo.time))}${Number.isFinite(Number(geo.accuracy))?` · Genauigkeit ${Math.round(Number(geo.accuracy))} m`:''}</span>`:`<b>Keine aktuelle Position</b><span>${!consent?'Zustimmung ist noch offen.':!sessionMatches?'Schicht ist nicht gestartet.':'GPS-Position wird noch ermittelt.'}</span>`}
