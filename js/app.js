// CORE-007D8A1F1D8P36F7W2 · 23.09.2026: WEB BUNDLE MANUAL CHECK BADGE FIX – Ergänzt den P36F7W1-Sicherheitsstatus für Bündelfahrten ohne aufgelösten Flugort: bei prüfpflichtigem Flug wird nun auch in Bündelkarten die Zeile "Flugort nicht verfügbar" mit sichtbarem "⚠ manuell prüfen" gerendert. Verifizierte Bündelflüge bleiben unverändert; Fahrten ohne Flugnummer erhalten weiterhin keinen Flugprüf-Hinweis. Keine Änderung an PLAN/DISPO/LIVE, OCR, Persistenz, GPS, Routing, Nachrichten, Fahrern oder Importlogik.\n// CORE-007D8A1F1D8P36F7W1 · 23.09.2026: WEB MANUAL CHECK SAFETY RESTORE – Stellt die FLIGHT-008-Sicherheitsanzeige in der Web-App wieder her: nur verified/high mit verifiziertem Flugort ist warnungsfrei. source_confirmed, unsichere/manuelle Prüfungen, Quellenkonflikte und Flüge ohne verifizierten Flugort zeigen sichtbar "⚠ manuell prüfen". PLAN/DISPO/LIVE, OCR, Persistenz, GPS, Routing, Nachrichten, Fahrer und bestehende Importlogik bleiben unverändert.
...
function rideCard(r,i){
  ...
  const bundleFlightLocation=r.isBundle&&(r.flightLocation||manualFlightCheck)?`<div class="flightloc" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:5px 0 4px"><span>✈ ${esc(r.flightLocation||'Flugort nicht verfügbar')}${r.iata?' ('+esc(r.iata)+')':''}</span><span style="font-size:12px;font-weight:800;padding:2px 7px;border-radius:7px;background:rgba(0,168,255,.15);border:1px solid rgba(0,168,255,.35);color:#16b8ff">${bundleFlightLabel}</span>${manualFlightBadge}</div>`:'';
