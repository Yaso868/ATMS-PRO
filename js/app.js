...
function currentClockMinutes(){const d=new Date();return d.getHours()*60+d.getMinutes()}
function minutesFromNow(target){if(!Number.isFinite(target)||target===99999)return null;let diff=target-currentClockMinutes();if(diff<-720)diff+=1440;if(diff>720)diff-=1440;return diff}
function pickupGapMinutes(fromRide,toRide){const a=minutesOf(effectiveTime(fromRide)),b=minutesOf(effectiveTime(toRide));if(a===99999||b===99999)return null;let gap=b-a;if(gap<0)gap+=1440;return gap}
function analyseLiveChain(drides,threshold){
  const warnGap=Math.max(30,threshold*3);
  return drides.map((ride,index)=>{
    const time=effectiveTime(ride);
    const delay=delayForRide(ride);
    if(!time||minutesOf(time)===99999)return{ride,status:'pending',label:'Zeit prüfen',detail:'Keine gültige Abholzeit vorhanden.',gap:null,kind:'missing-time'};
    if(delay>=threshold)return{ride,status:'bad',label:`+${delay} Min. verspätet`,detail:'Gemeldete Verspätung liegt über der Warnschwelle.',gap:null,kind:'delay',delay};
    if(delay>0)return{ride,status:'warn',label:`+${delay} Min. gemeldet`,detail:'Gemeldete Verspätung unterhalb der Warnschwelle.',gap:null,kind:'delay',delay};
    if(index===0){
      const delta=minutesFromNow(minutesOf(time));
      if(delta===null)return{ride,status:'pending',label:'Zeit prüfen',detail:'Aktuelle Zeitprüfung nicht möglich.',gap:null,kind:'missing-time'};
      if(delta<=-threshold)return{ride,status:'bad',label:`${Math.abs(delta)} Min. überfällig`,detail:`Die aktuelle Abholzeit ${time} ist bereits überschritten.`,gap:null,kind:'overdue',overdue:Math.abs(delta)};
      if(delta<0)return{ride,status:'warn',label:`seit ${Math.abs(delta)} Min. fällig`,detail:`Die aktuelle Abholzeit ${time} wurde gerade überschritten.`,gap:null,kind:'overdue',overdue:Math.abs(delta)};
      if(delta<=threshold)return{ride,status:'warn',label:`in ${delta} Min.`,detail:'Abholung steht unmittelbar bevor.',gap:null,kind:'upcoming'};
      return{ride,status:'good',label:`in ${delta} Min.`,detail:'Aktuelle Fahrt liegt zeitlich vor der Warnschwelle.',gap:null,kind:'upcoming'};
    }
    const gap=pickupGapMinutes(drides[index-1],ride);
    if(gap===null)return{ride,status:'pending',label:'Abstand prüfen',detail:'Zeitabstand zur vorherigen Fahrt konnte nicht berechnet werden.',gap:null,kind:'gap'};
    if(gap<=threshold)return{ride,status:'bad',label:`nur ${gap} Min. Abstand`,detail:'Sehr geringer Abstand zwischen den Abholzeiten. Routenzeit unbedingt prüfen.',gap,kind:'gap'};
    if(gap<=warnGap)return{ride,status:'warn',label:`${gap} Min. Abstand`,detail:'Knappes Zeitfenster zwischen den Abholzeiten. Routenzeit prüfen.',gap,kind:'gap'};
    return{ride,status:'good',label:`${gap} Min. Abstand`,detail:'Zeitabstand zwischen den Abholzeiten ist unauffällig. Fahrtdauer ist noch nicht eingerechnet.',gap,kind:'gap'};
  })
}
...
function renderRouteCheck(driver,drides,threshold){
  ...
  const s=getLiveSettings(),geo=(s.lastGeo&&s.lastGeo.driverId===driver.id)?s.lastGeo:null;
  const notice=$('livePhoneDriverNotice');if(notice)notice.innerHTML=`<b>Zuordnung:</b> Die GPS-Position dieses Handys wird ausschließlich für die Fahrten von <b>${esc(driver.name)}</b> verwendet. Bitte stelle sicher, dass ${esc(driver.name)} dieses Gerät verwendet.`;
  const analysis=analyseLiveChain(drides,threshold),chain=[];
  ...
  analysis.slice(0,6).forEach((item,i)=>{const r=item.ride,stepClass=item.status==='bad'||item.status==='warn'?'warn':item.status==='good'?'ok':'pending';const routeText=r.isBundle?routeLabelForRide(r):`${r.pickup||'Abholort fehlt'} → ${r.destination||'Ziel fehlt'}`;chain.push(`<div class="route-step ${stepClass}"><span class="step-icon">${i+2}</span><div><b>${i===0?'Aktuelle Fahrt':'Folgefahrt '+i}</b><small>${esc(effectiveTime(r)||'–')} · ${esc(routeText)}</small><small>${esc(item.detail)}</small></div><em>${esc(item.label)}</em></div>`)});
  $('liveRouteChain').innerHTML=chain.join('');
  const bad=analysis.filter(x=>x.status==='bad').length,warn=analysis.filter(x=>x.status==='warn').length,good=analysis.filter(x=>x.status==='good').length;
  $('liveRouteSummary').innerHTML=`<div><small>UNAUFFÄLLIG</small><b style="color:#59ef8b">${good}</b></div><div><small>PRÜFEN</small><b style="color:#ffc95a">${warn}</b></div><div><small>KRITISCH</small><b style="color:#ff7189">${bad}</b></div>`;
  $('liveGeoState').innerHTML=geo?`<b>Standort dieses Handys verwendet:</b> ${Number(geo.lat).toFixed(5)}, ${Number(geo.lng).toFixed(5)} · ${esc(driver.name)} zugeordnet · zuletzt ${new Date(geo.time).toLocaleTimeString('de-DE')}<br><small>LIVE-001B.1 bewertet Zeitabstände. Echte Fahrtdauer/Verkehr folgt mit Routing-API.</small>`:`<b>Standort dieses Handys:</b> noch nicht ermittelt. Tippe auf den Button und erlaube den Standortzugriff. Die Position wird danach ${esc(driver.name)} zugeordnet.<br><small>Zeitkettenprüfung funktioniert auch ohne GPS; echte Fahrtdauer wird noch nicht behauptet.</small>`;
  analysis.filter(x=>x.kind==='delay'&&x.status==='bad').forEach(x=>ensureLiveDelayEvent(driver,x.ride,threshold));
}
