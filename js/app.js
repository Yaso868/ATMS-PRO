...
async function parseAddressBookXlsx(file){
  ...
  const parser=new DOMParser(),wb=parser.parseFromString(await wbFile.async('text'),'application/xml'),sheet=wb.getElementsByTagName('sheet')[0];if(!sheet)throw new Error('Excel-Datei enthält kein Arbeitsblatt.');const relId=sheet.getAttribute('r:id')||sheet.getAttributeNS('http://schemas.openxmlformats.org/officeDocument/2006/relationships','id');
  const rels=parser.parseFromString(await relsFile.async('text'),'application/xml');let target='';for(const rel of[...rels.getElementsByTagName('Relationship')])if(rel.getAttribute('Id')===relId){target=rel.getAttribute('Target')||'';break}if(!target)target='worksheets/sheet1.xml';target=target.replace(/^\/?xl\//,'').replace(/^\//,'');const sheetFile=zip.file('xl/'+target.replace(/^\.\//,''));if(!sheetFile)throw new Error('Erstes Excel-Arbeitsblatt konnte nicht gelesen werden.');
  let shared=[];const sharedFile=zip.file('xl/sharedStrings.xml');if(sharedFile){const doc=parser.parseFromString(await sharedFile.async('text'),'application/xml');shared=[...doc.getElementsByTagName('si')].map(si=>[...si.getElementsByTagName('t')].map(t=>t.textContent||'').join(''))}
  const doc=parser.parseFromString(await sheetFile.async('text'),'application/xml'),matrix=[];for(const rowEl of[...doc.getElementsByTagName('row')]){const row=[];for(const c of[...rowEl.getElementsByTagName('c')]){const ref=c.getAttribute('r')||'',letters=(ref.match(/[A-Z]+/i)||['A'])[0].toUpperCase();let idx=0;for(const ch of letters)idx=idx*26+(ch.charCodeAt(0)-64);idx=Math.max(0,idx-1);const type=c.getAttribute('t')||'';let value='';if(type==='inlineStr')value=[...c.getElementsByTagName('t')].map(t=>t.textContent||'').join('');else{const v=c.getElementsByTagName('v')[0]?.textContent||'';value=type==='s'?String(shared[Number(v)]??''):v}row[idx]=value}matrix.push(row)}return addressRowsFromMatrix(matrix)
}
